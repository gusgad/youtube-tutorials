...
import { createRng, jitterMs } from "./jitter.js";
import { toInputMessages, toOutputMessages } from "./genai_messages.js";

const MODEL = "claude-sonnet-5";
const CONVERSATION_ID = newConversationId();
const tracer = initTracer("payment-demo-fleet-backoff");

const ORDER_IDS = [1, 2, 3, 4, 5, 6];

const MAX_ATTEMPTS = 4;
const INITIAL_STAGGER_MAX_MS = 21000;
const BACKOFF_BASE_MS = 2400;
const BACKOFF_JITTER_SPREAD_MS = 2400;
...
function buildChargeCardTool({ tracer, conversationId, agentName, agentResult, rng }) {
  return tool(
    ({ order_id, amount_cents }) =>
      tracer.startActiveSpan("charge_card", async (span) => {
        try {
          ...
          // Stagger the very first attempt too -- otherwise all six agents'
          // "attempt 1" would still land in the same instant, and the fix
          // would only ever kick in after the first guaranteed collision.
          const initialStagger = jitterMs(rng, 0, INITIAL_STAGGER_MAX_MS);
          ...
          for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            ...
            if (attempt < MAX_ATTEMPTS) {
              const backoff = jitterMs(
                rng,
                BACKOFF_BASE_MS * 2 ** (attempt - 1),
                BACKOFF_BASE_MS * 2 ** (attempt - 1) + BACKOFF_JITTER_SPREAD_MS
              );
              console.log(
                `  [agent-${order_id}] attempt ${attempt} rejected (at capacity), backing off ${Math.round(backoff)}ms...`
              );
              await sleep(backoff);
            }
          }
...
async function runAgent(orderId) {
  ...
  const rng = createRng(1000 + orderId);
